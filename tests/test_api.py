...
class TestAPI:
    ...
    def test_query_routine_symptoms(self):
        ...
        response = client.post(
            "/query",
            json={"device_id": "test-device-001", "symptoms": ["mild headache", "runny nose"]},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_query_emergency_symptoms(self):
        ...
        response = client.post(
            "/query",
            json={"device_id": "test-device-003", "symptoms": ["severe chest pain", "syncope"]},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_query_with_vitals(self):
        ...
        response = client.post(
            ...
            json={
                ...
                "symptoms": ["fever"],
                "vitals": {"temperature_f": 102.0, "heart_rate": 90, "oxygen_saturation": 96.0},
            },
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_validate_sanitization_clean(self):
        ...
        response = client.post(
            "/validate-sanitization", params={"text": "Patient has fever and cough"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_validate_sanitization_phi_detected(self):
        ...
        response = client.post(
            "/validate-sanitization", params={"text": "Contact john@example.com for records"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_query_returns_fingerprint(self):
        ...
        response = client.post(
            "/query", json={"device_id": "test-device-006", "symptoms": ["cough"]}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_query_empty_symptoms(self):
        ...
        response = client.post("/query", json={"device_id": "test-device-008", "symptoms": []})
